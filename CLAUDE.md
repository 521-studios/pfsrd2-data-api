...
GET /search/suggest/unified?q=orc&type=monsters  (edition-aware, with alternates; also takes traits/category/subcategory + level_min/level_max; q may be empty when a filter is present → filter-only browse)
...
POST /entries/{item_game_id}/apply/{effect_game_id}?grade=N  → same, but applies onto the in-progress item in the body (stacking)?version=1.3
